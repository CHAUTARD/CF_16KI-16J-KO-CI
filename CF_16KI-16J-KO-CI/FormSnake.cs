...
namespace CF_16KI_16J_KO_CI
{
    public partial class FormSnake : Form
    {
        ...
        { 
            dgv.Rows.Add(sRang, SingletonOutils.lJoueurs[j].GetNom(), SingletonOutils.lJoueurs[j].Dossard, SingletonOutils.lJoueurs[j].Club);
        ...
        private void EditionPoule(char cPoule)
        {
            ...
            // 2 joueurs
            if (SingletonOutils.lJoueurs[iGroupe[2]].GetNom().Length == 0)
            {
                iMax = 1;
            ...
            else
            {
                // 3 Joueurs
                // 1-3, 2-3, 1-2
                if (SingletonOutils.lJoueurs[16].GetNom().Length == 0)
                {
                    iMax = 3;
        ...
        private void UpdateJoueurPouleNum(char cPoule, int iNum, int iDossard)
        {
            SQLiteCommand sqlCommand = new SQLiteCommand("UPDATE joueurs SET `Poule` = '" + cPoule.ToString() + "', NumInPoule = " + iNum.ToString() + " WHERE Dossard=" + iDossard.ToString(), SingletonOutils.sqliteConn);
        ...
        private void btnPoules_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvGroupe1.Rows.Count; i++)
                UpdateJoueurPouleNum('A', i + 1, (int) dgvGroupe1.Rows[i].Cells[2].Value);

            for (int i = 0; i < dgvGroupe2.Rows.Count; i++)
                UpdateJoueurPouleNum('B', i + 1, (int)dgvGroupe2.Rows[i].Cells[2].Value);


            for (int i = 0; i < dgvGroupe3.Rows.Count; i++)
                UpdateJoueurPouleNum('C', i + 1, (int)dgvGroupe3.Rows[i].Cells[2].Value);

            for (int i = 0; i < dgvGroupe4.Rows.Count; i++)
                UpdateJoueurPouleNum('D', i + 1, (int)dgvGroupe4.Rows[i].Cells[2].Value);
        ...
        private void ficheDesPoulesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (ExcelPackage p = new ExcelPackage(new FileInfo(sFileName)))
            {
                ...
                for (int i = 1; i < 5; i++)
                {
                    ...
                    {
                        ws.Cells[iLigne, 2].Value = SingletonOutils.lJoueurs[iJ].GetNom();
                        ws.Cells[iLigne, 3].Value = int.Parse(SingletonOutils.lJoueurs[iJ].Dossard.ToString());
                        ws.Cells[iLigne, 4].Value = int.Parse(SingletonOutils.lJoueurs[iJ].Licence.ToString());
            ...
            using (var p = new ExcelPackage(sFileName))
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    foreach (int iJ in SingletonOutils.GetJoueurPoule4(Convert.ToChar(i + 65)))
                    {
                        ...
                        ws.Cells[iR2, iC + 1].Value = SingletonOutils.lJoueurs[iJ].GetNom();
                        ws.Cells[iR2, iC + 3].Value = int.Parse(SingletonOutils.lJoueurs[iJ].Licence.ToString());
                    ...
                    for (int j = 0; j < 6; j++)
                    {
                        ...
                        ws.Cells[iR + j + 6, iC + 1].Value = SingletonOutils.lJoueurs[iPos].GetNom();

                        iPos = SingletonOutils.GetJoueurPoule4(Convert.ToChar(i + 65))[iJoueur2 - 1];
                        ws.Cells[iR + j + 6, iC + 2].Value = SingletonOutils.lJoueurs[iPos].GetNom();
                    }
                }
